...
class PinyinConverter implements ConverterInterface
{
    ...
    private function checkAndExecuteMigration()
    {
        ...
        if (($currentTime - $lastMigrationTime) >= $migrationFrequency) {
            ...
            if ($this->config['background_tasks']['enable_logging'] ?? false) {
                error_log(
                    "[PinyinConverter] 字典迁移检查完成，下次检查时间: " .
    ...
    {
        $path = $this->config['dict']['backup'] . "/last_migration.txt";
        write_to_file($path, (string)time());
    }
    /**
     * 按类型和优先级加载字典
     *
    ...
    private function convertPinyinTone($sourcePinyin, $sourceToneType, $targetToneType, $char)
    {
        ...
        foreach ($sourcePinyin as $pinyin) {
            ...
            } elseif ($sourceToneType === 'no_tone' && $targetToneType === 'with_tone') {
                // 无声调转带声调：需要复杂的查找逻辑
        ...
        foreach ($this->dicts['polyphone_rules'][$char] as $existingRule) {
            ...
            if (isset($existingRule['type']) && $existingRule['type'] === $rule['type']) {
                ...
                if ($rule['type'] === 'word' && isset($existingRule['word'], $rule['word']) && $existingRule['word'] === $rule['word']) {
                    $ruleExists = true;
                    break;
                } elseif ($rule['type'] === 'pre' && isset($existingRule['char'], $rule['char']) && $existingRule['char'] === $rule['char']) {
                    $ruleExists = true;
                    break;
                } elseif ($rule['type'] === 'post' && isset($existingRule['char'], $rule['char']) && $existingRule['char'] === $rule['char']) {
                    $ruleExists = true;
                    break;
                } elseif ($existingRule === $rule) { // 对于其他类型的规则，我们比较整个规则数组
                    $ruleExists = true;
                    break;
                }
            }
    ...
    private function parseCharParam($specialCharParam)
    {
        ...
        // 默认返回值
        $result = ['mode' => $defaultMode, 'map' => []];

        // 字符串类型处理
        if (is_string($specialCharParam)) {
            $result['mode'] = in_array($specialCharParam, ['keep', 'delete', 'replace']) ? $specialCharParam : $defaultMode;
        } elseif (is_array($specialCharParam)) { // 数组类型处理
            $result['mode'] = isset($specialCharParam['mode']) && in_array($specialCharParam['mode'], ['keep', 'delete', 'replace'])
                ? $specialCharParam['mode']
                : $defaultMode;
            $result['map'] = isset($specialCharParam['map']) && is_array($specialCharParam['map'])
    ...
    public function getUrlSlug($text, $separator = '-')
    {
        ...
        $enableChineseToArabic = getenv('ENABLE_CHINESE_TO_ARABIC') === false ? false : true;
